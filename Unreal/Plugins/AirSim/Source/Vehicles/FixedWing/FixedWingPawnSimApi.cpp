...
void FixedWingPawnSimApi::initialize()
{
    ...
    createVehicleApi(static_cast<AFixedWingPawn*>(params_.pawn), params_.home_geopoint);

    //TODO: should do reset() here?
    joystick_controls_ = FixedWingPawnApi::FixedWingControls();
}

void FixedWingPawnSimApi::createVehicleApi(AFixedWingPawn* pawn, const msr::airlib::GeoPoint& home_geopoint)
{
    //create vehicle params
    std::shared_ptr<UnrealSensorFactory> sensor_factory = std::make_shared<UnrealSensorFactory>(getPawn(), &getNedTransform());
    vehicle_api_ = std::unique_ptr<FixedWingApiBase>(new FixedWingPawnApi(pawn, getGroundTruthKinematics(), home_geopoint,
        getVehicleSetting(), sensor_factory, 
        (*getGroundTruthKinematics()), (*getGroundTruthEnvironment())));
}

std::string FixedWingPawnSimApi::getRecordFileLine(bool is_header_line) const
{
    ...
    const auto state = vehicle_api_->getFixedWingState();
...
void FixedWingPawnSimApi::updateFixedWingControls()
{
    ...
    if (rc_data.is_initialized) {
        ...
        if (rc_data.vendor_id == "VID_044F") {
            ...
            auto car_state = vehicle_api_->getFixedWingState();
    ...
    if (!vehicle_api_->isApiControlEnabled()) {
        ...
        vehicle_api_->setFixedWingControls(current_controls_);
    }
    else {
        UAirBlueprintLib::LogMessageString("Control Mode: ", "API", LogDebugLevel::Informational);
        current_controls_ = vehicle_api_->getFixedWingControls();
    }
...
void FixedWingPawnSimApi::reset()
{
    PawnSimApi::reset();
