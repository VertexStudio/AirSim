...
void ASimModeFixedWing::initializePauseState()
{
    pause_period_ = 0;
...
bool ASimModeFixedWing::isPaused() const
{
    return current_clockspeed_ == 0;
}

void ASimModeFixedWing::pause(bool is_paused)
{
    if (is_paused)
...
void ASimModeFixedWing::continueForTime(double seconds)
{
...
void ASimModeFixedWing::setupClockSpeed()
{
    current_clockspeed_ = getSettings().clock_speed;
...
void ASimModeFixedWing::Tick(float DeltaSeconds)
{
...
{
    return vehicle_type == AirSimSettings::kVehicleTypePhysXFixedWing;
}
...
const common_utils::UniqueValueMap<std::string, APIPCamera*> ASimModeFixedWing::getVehiclePawnCameras(
    APawn* pawn) const
{
...
std::unique_ptr<PawnSimApi> ASimModeFixedWing::createVehicleSimApi(
...
{
    const auto car_sim_api = static_cast<const FixedWingPawnSimApi*>(sim_api);
